...
#
# Different job types require different input _files. For example, an Excel import can use a
# 'template' and one or more separate 'data' _files, or 'combined' _files which include both the data
# and the template. Any additional _files to be imported as file or picture attributes should be
# specified as 'attachment' _files. These can be provided as relative or absolute paths or as
...
separate_excel_import_request = ExcelImportJobRequest(
    name="Excel Import (separate template and data _files)",
...
# ## Access output _files
# Finally, access the results of the job. Import jobs typically create log _files, but the exact type
# of _files generated varies based on the type of import template. In this case, the _files are all
# plain text.

# Access the list of _files generated by the job with the ``output_file_names`` property. This
# returns a list of file names.

completed_job.output_file_names

# In general, an Excel import job will return two _files:
#
